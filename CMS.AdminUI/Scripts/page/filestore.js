...
$(function () {
    $('#fileList').datagrid({
        ...
        columns: [[
                ...
                { field: 'ExtName', title: '类型', width: 100, sortable: true },
                {
                ...
                {
                    field: 'FileURL', title: '链接地址', width: 100, sortable: true, formatter: function (value, rec) {
                        return '<a href="#" onclick="copyToClipBoardhtml(\'' + value + '\')">复制链接</a>' +
                        '<a href="javascript:void(0);" onclick="SendMailInit(1,\'' + value + '\')"></a>';
                    }
                }
        ]],
...
function addFileMapPath(path) {
    ...
    var smstr = "<span class=\"cc" + index + "\">>></span><input type=\"hidden\" value=\"" + ypath + "\" id=\"sc"+index+"\"><a href=\"#\" class=\"easyui-linkbutton l-btn l-btn-plain\" plain=\"true\" onclick=\"javascript:fileMapControl(" + index + ");\" id=\"sm" + index + "\" >" +
        "<span class=\"l-btn-left\"><span class=\"l-btn-text\">" + path + "</span></span></a>";
    $("#smc").append(smstr);
}
    ...
    $('#dialogview').dialog({
        ...
        buttons: [{
            ...
            handler: function () {
                ...
                var _currentPath = $("#smc input:last").val() + "\\" + newFold;
                $.ajax({
...
function UpdloadFiles() {
    ...
    $('#UploadFilesDialog').dialog({
        buttons: [{
            ...
            handler: function () {            
                if (CheckUpImg()) {
                    var curpath = $("#smc input:last").val();
                    $('#uploadForm').ajaxSubmit({
                        ...
                        success: function (data) {
                            ...
                            if (jsonObj.result == "ok") {
                                ...
                                $('#fileList').datagrid({
                                    url: '/FileStore/AjaxJson?localPath=' + curpath
                                });
                            }
